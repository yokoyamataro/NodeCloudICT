...
import { OpenChannelLayer } from './OpenChannelLayer'
...
export function UnifiedFieldMap({ baseLayer = 'osm', layers, farmId, children }: UnifiedFieldMapProps) {
  ...
  // 線形物 (open channel) の 中心線 / 幅杭 / 線形点 (BP/IP/EP)。
  // 生成も 描画も スマホの 測設画面と 共有する (見た目を 揃えるため)。
  ...
  return (
    <MapContainer center={initialCenter} zoom={15} maxZoom={24} className="h-full w-full">
      ...
      {layers.openChannels && <OpenChannelLayer renders={openChannelRenders} />}
